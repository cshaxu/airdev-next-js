...
const AWS_CLIENT_CONFIG = {
  region: privateAppConfig.aws.s3Region,
  credentials: {
    accessKeyId: privateAppConfig.aws.accessKeyId,
    secretAccessKey: privateAppConfig.aws.secretAccessKey,
  },
};
...
const isDataLocal = publicAppConfig.service.dataEnvironment === 'local';

export async function getS3Object(
  key: string
): Promise<GetObjectCommandOutput> {
  const payload = { Bucket: publicAppConfig.aws.s3Bucket, Key: key };
  if (isDataLocal) {
...
export async function uploadS3Object(
  ...
  if (isDataLocal) {
    ...
    formData.append('Bucket', publicAppConfig.aws.s3Bucket);
    ...
    const command = new PutObjectCommand({
      Bucket: publicAppConfig.aws.s3Bucket,
      Key: key,
      Body: file,
    });
    return await s3Client.send(command);
...
export async function startDocumentTextDetection(
  ...
  const payload = {
    DocumentLocation: {
      S3Object: { Bucket: publicAppConfig.aws.s3Bucket, Name: key },
    },
  };
  if (isDataLocal) {
    return await fetch(
