import { mockContext } from '../../../utils/context';
import { buildNextauthUserFromPackageUser } from '../../../utils/user';
...
export const codeProvider = CredentialsProvider({
    ...
    async authorize(credentials) {
        ...
        const nextauthVerificationToken = await backendFunctionConfig.nextauthVerificationToken.deleteOneSafe(pick(credentials, ['email', 'code']), context);
        if (nextauthVerificationToken === null) {
            return null;
        }
        const { identifier: email } = nextauthVerificationToken;
        const user = await backendFunctionConfig.user.getOrCreateOne(email, context);
        const verifiedUser = await backendFunctionConfig.user.updateOne(user.id, { emailVerified: context.time }, context);
