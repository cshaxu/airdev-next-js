...
import { measureElementWidth } from '../../utils/element.js';
...
export function ResponsiveBreadcrumb({ items, className, renderItem }) {
    ...
    // 计算分隔符宽度
    const measureSeparatorWidth = useCallback(() => {
        if (!measureRef.current) {
            return 0;
        }
        return measureElementWidth(() => createSeparatorElement(), measureRef.current);
    }, []);
    // 计算省略号宽度
    const measureEllipsisWidth = useCallback(() => {
        if (!measureRef.current) {
            return 0;
        }
        return measureElementWidth(() => createEllipsisElement(), measureRef.current);
    }, []);
    // 更新显示的项目
    const updateDisplayedItems = useCallback((containerWidth) => {
        ...
        const itemWidths = items.map((item) => item === ELLIPSIS_ITEM
            ...
            : measureElementWidth(() => createItemElement(item), measureRef.current));
    ...
    return (_jsx("div", { ref: containerRef, className: `w-full min-w-[50px] ${className || ''}`, children: _jsx(Breadcrumb, { children: _jsx(BreadcrumbList, { className: "flex-nowrap", children: displayedItems.map((item, index) => {
                    const isLast = index === displayedItems.length - 1;
                    const isEllipsis = item === ELLIPSIS_ITEM;
                    return (_jsxs(React.Fragment, { children: [_jsx(BreadcrumbItem, { "data-item-index": index, className: "whitespace-nowrap", children: isEllipsis ? (_jsxs(DropdownMenu, { children: [_jsx(DropdownMenuTrigger, { className: "hover:text-foreground flex cursor-pointer items-center", children: _jsx(BreadcrumbEllipsis, { className: "text-muted-foreground/50" }) }), _jsx(DropdownMenuContent, { align: "start", children: hiddenItems.map((hiddenItem, hiddenIndex) => (_jsx(DropdownMenuItem, { asChild: !!hiddenItem.href, children: hiddenItem.href ? (_jsx(BreadcrumbLink, { href: hiddenItem.href, className: "text-muted-foreground hover:text-foreground w-full cursor-pointer truncate", children: renderItem
                                                        ? renderItem(hiddenItem)
                                                        : hiddenItem.label })) : (_jsx("span", { className: "text-muted-foreground w-full truncate", children: renderItem
                                                        ? renderItem(hiddenItem)
                                                        : hiddenItem.label })) }, hiddenIndex))) })] })) : (_jsx("div", { className: "truncate", children: isLast ? (_jsx(BreadcrumbPage, { className: "truncate", children: renderItem ? renderItem(item) : item.label })) : (_jsx(BreadcrumbLink, { href: item.href, className: "text-muted-foreground hover:text-foreground truncate", children: renderItem ? renderItem(item) : item.label })) })) }), !isLast && (_jsx(BreadcrumbSeparator, { "data-measure-target": "breadcrumb-separator", className: "text-muted-foreground/50 mx-0.5 [&>svg]:h-3.5 [&>svg]:w-3.5" }))] }, item.label || index));
                }) }) }) }));
}
// Utils
