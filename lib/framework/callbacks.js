...
import { HEADER_AUTHORIZATION_KEY, HEADER_INTERNAL_SECRET_KEY, publicConfig, } from '@/config/public';
...
function authorizer(context, options) {
    ...
    if (options?.requireCron === true) {
        if (context.headers.get(HEADER_AUTHORIZATION_KEY) === `Bearer ${CRON_SECRET}`) {
            return;
        }
