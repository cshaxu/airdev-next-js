...
import { MintTooltip } from '@/frontend/components/ui/Tooltip';
import { ACCEPTED_FILE_TYPES, MAX_FILE_SIZE } from '@/frontend/constants';
import useGetBotMessageStream from '@/frontend/hooks/custom/get-bot-message-stream';
import { useUploadFiles } from '@/frontend/hooks/useUploadFiles';
...
export default function ChatInput({
  ...
  const { resetCurrentAttachments, addCurrentAttachment } =
    useCurrentChatActions();
  ...
  const onUpload = async (file: File, id: string) => {
    addCurrentAttachment({
      type: ChatMessageAttachmentType.FILE,
      id,
      originalName: file.name,
    });
  };

  const { handleUpload, isUploading } = useUploadFiles({ onUpload });

  const onDrop = useCallback(
    async (acceptedFiles: File[]) => {
      const uploadedFiles = await handleUpload(acceptedFiles);
      logInfo(uploadedFiles);
    },
    [handleUpload]
  );
    ...
    <form onSubmit={handleSubmit} className={cn('p-4', className)}>
      ...
      >
        ...
        <div className="mt-1 mb-2 flex items-center justify-between">
          <MintTooltip tip="Add files to your chat">
            <div {...getRootProps()}>
              <Button
                type="button"
                variant="outline"
