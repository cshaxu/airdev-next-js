...
export type GetOneNextauthAccountParams = Pick<
  DatabaseNextauthAccount,
  'provider' | 'providerAccountId'
>;

export type UpdateOneNextauthAccountBody = Omit<DatabaseNextauthAccount, 'id'>;

export type CreateOneNextauthSessionBody = Pick<
  DatabaseNextauthSession,
...
export type DatabaseAdapter = {
  getOneUserSafe: (
    ...
    context: Context
  ) => Awaitable<DatabaseUser | null>;
  createOneUser: (
    body: CreateOneUserBody,
    context: Context
  ) => Awaitable<DatabaseUser>;
  updateOneUser: (
    one: DatabaseUser,
  ...
  getOneNextauthAccountSafe: (
    params: GetOneNextauthAccountParams,
    context: Context
  ) => Awaitable<DatabaseNextauthAccount | null>;
  updateOneNextauthAccount: (
    one: DatabaseNextauthAccount,
    body: UpdateOneNextauthAccountBody,
    context: Context
  ) => Awaitable<DatabaseNextauthAccount>;
  createOneNextauthSession: (
    body: CreateOneNextauthSessionBody,
    context: Context
  ) => Awaitable<DatabaseNextauthSession>;
  deleteOneNextauthVerificationTokenSafe: (
    params: GetOneNextauthVerificationTokenParams,
    context: Context
  ) => Awaitable<DatabaseNextauthVerificationToken | null>;
  getOneRequestCache: (
    body: DatabaseRequestCacheBody,
    context: Context
  ) => Awaitable<DatabaseRequestCache>;
  createOneRequestCacheSafe: (
    body: DatabaseRequestCacheBody,
    context: Context
  ) => Awaitable<DatabaseRequestCache | null>;
  updateOneRequestCacheSafe: (
    requestCache: DatabaseRequestCache,
    response: unknown,
    context: Context
  ) => Awaitable<void>;
};
