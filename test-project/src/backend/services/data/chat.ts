...
// config imports
import { adminOrThrow, Context, selfOrThrow } from '@/framework/context';
...
const beforeCreateOne = (body: CreateOneChatBody, context: Context): void =>
  selfOrThrow(context, body.userId);

async function createOne(
  body: CreateOneChatBody,
  context: Context
): Promise<ChatEntity> {
  const userId = typeof body === 'string' ? body : body.userId;
  const one = await ChatEntity.create(
    {
      data: {
