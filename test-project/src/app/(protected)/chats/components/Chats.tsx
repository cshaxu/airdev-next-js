...
import { useCreateOneChat } from '@/generated/tanstack-hooks/chat-client';
...
export default function Chats() {
  ...
  const createChat = useCreateOneChat({
      ...
      <HeaderBar
        ...
        actions={
          <Button
            ...
            onClick={() => createChat.mutate({ userId: currentUser.id })}
            ...
            {chatUsers.length === 0 ? (
              <Card className="rounded-[28px] border-dashed border-[var(--blue-separator)] bg-[var(--blue-bg)] py-10 text-center shadow-none">
                ...
                <CardContent className="flex justify-center">
                  <Button
                    onClick={() =>
                      createChat.mutate({ userId: currentUser.id })
                    }
                    isLoading={createChat.isPending}
                  >
                    <Plus className="size-4" />
